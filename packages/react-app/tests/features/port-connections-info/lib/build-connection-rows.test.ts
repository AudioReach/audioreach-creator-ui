...
describe('buildConnectionRows', () => {
  ...
  it('maps one row per input dto, in order', () => {
    const dtoA = makeDataLink({sourceSystemId: 'sys-mod-1', systemId: 'link-a'});
    const dtoB = makeDataLink({sourceSystemId: 'sys-mod-1', systemId: 'link-b'});
  ...
  it('keeps distinct usecases within a single link intact', () => {
    ...
    const dto = makeDataLink({sourceSystemId: 'sys-mod-1'}, [usecaseA, usecaseB]);
    const [row] = buildConnectionRows([dto], [], 'sys-mod-1');
