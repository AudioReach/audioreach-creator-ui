...
| Right-click port    | Data: "Start connection" / "Start EC Link" / "Start Dangling Data Link"; control: "Start connection" / "Start Dangling Control Link"; completion uses "End connection" for normal links and matching EC/Dangling completion items — required when the target port isn't yet rendered (e.g. into a collapsed subsystem) |
