...
| Right-click port    | Data: "Start connection" / "Start EC Link" / "Start InterUsecase Data Link"; control: "Start connection" / "Start InterUsecase Control Link"; completion uses "End connection" for normal links and matching EC/InterUsecase completion items — required when the target port isn't yet rendered (e.g. into a collapsed subsystem) |
