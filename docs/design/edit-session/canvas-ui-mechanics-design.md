...
| `'port'`                                     | Data ports show "Start connection", "Start EC Link", and "Start Dangling Data Link" when idle; control ports show "Start connection" and "Start Dangling Control Link". An active connection shows exactly one matching completion item. See [link-and-port-design.md §2.2](link-and-port-design.md#22-two-click-state-visualizer-internal) for why this field is populated by the Visualizer itself rather than read from its store here. |
...
| `'proxy-data-link'` / `'proxy-control-link'` | Same as the plain edge modes — a proxy edge is the display-collapsed form of a real link (FR-MDF-02), not a distinct excludable entity                                                                                                                                                                                                                                                                                                                                                                                 |
...
- Two-click connection state (`connectionInProgress`) — Visualizer-internal
  ...
  only _reads_ `target.connectionInProgress`, a `{edgeMode}` object or `null` that the
...
- **Unit — context menu item visibility**: data ports expose Start connection,
  Start EC Link, and Start Dangling Data Link; control ports expose Start
  connection and Start Dangling Control Link; the matching completion item
